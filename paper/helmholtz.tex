In the previous section, we stated that modifying the composition of the medium
is equivalent to modifying the geometry that light perceives. What does this
mean mathematically? This section provides two examples using the Helmholtz
equation. We will see precisely how this statement translates into mathematics.
This represents the fundamental idea behind cloaking: a change of coordinates
can be encoded into the material properties of the medium.

\subsection{Set up}
We will work with the Helmholtz equation in \(\mathbb{R}^2\)
\begin{equation}\label{helmholtz}
	(\Delta +k^2)f=(\partial_x^2+\partial_y^2+k^2)f=0.
\end{equation}
We can interpret \(k\) as a property of the medium, such as the refractive
index. Although technically \(k\) represents the wave number, for our purposes
we can treat it as the refractive index. We will demonstrate how a coordinate
transformation can be encoded into this parameter \(k\), or equivalently, how
tweaking this parameter induces a geometric transformation.

\subsection{Example 1: stretching the space}
Consider the Helmholtz equation \eqref{helmholtz} and apply the following
change of variables
\begin{equation*}
	u=3x \qquad
	v=3y.
...
\end{equation*}
This result shows that solving the transformed equation is equivalent to solving
the original Helmholtz equation \eqref{helmholtz} with \(k\) replaced by
\(k/3\). In other words, stretching the space by a factor of 3 is equivalent to
dividing \(k\) by 3. This example demonstrates that modifying the medium's
composition and changing the geometry that light perceives produce equivalent
effects.

\subsection{Example 2: inverting with respect to the unit circle}
...
Using \eqref{partialsRelations} to eliminate the \(v\) variable from
\eqref{laplacianuv}, we obtain
...
complex than in the previous case; however, it remains a function of space
alone, depending only on \(x\) and \(y\) as shown in \eqref{partialu}.

\subsection{A counterexample}
Not all transformations can be realized by modifying \(k\). For example,
consider
\begin{equation*}
	u=2x \qquad
	v=3y.
