In the previous section we said that modifying the composition of the medium is
equivalent to modifying the geometry that light perceives; what does this mean
mathematically? This section aims to provide two examples of this, using the
Helmholtz equation. We will see precisely what this statement means
mathematically. This is the fundamental idea behind cloaking, we will see how a
change of coordinates can be encoded into the material properties of the
medium.

\subsection{Set up}
We will be working with the Helmholtz equation in \(\mathbb{R}^2\)
\begin{equation}\label{helmholtz}
	(\Delta +k^2)f=(\partial_x^2+\partial_y^2+k^2)f=0.
\end{equation}
Here we can think of \(k\) as a property of the medium, for example the
refractive index. Even though it is technically the wave number, for our
purposes we can think of it as the refractive index. We will see how a
coordinate transformation can be encoded into this parameter \(k\), or
equivalently how by tweaking this parameter \(k\) we can induce a geometric
transformation.
