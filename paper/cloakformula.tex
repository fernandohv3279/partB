...
We are now ready to use the formula, in our case we have
\[\Lambda=\text{diag}\left( \frac{b-a}{b} ,1,1\right)\]
as our Jacobian matrix.
Our physical and virtual space are described in cylindrical
...
\right).
\]
The last step we have to do in mainly for convenience.
In short, the eigenvalues of the matrix \(\varepsilon^{ij}\) depend on the
coordinate system, so it is convenient to express them in terms of
Cartesian coordinates. Using some tools from linear algebra, whic
will not be explained here (the full explanation can be found in
section 5 of \cite{leonhardtGeometryLightScience2010}) it is
possible to prove that by lowering an index with the metric tensor
we obtain a matrix whose eigenvalues (when computed in an arbitrary
coordinate system) ate the eigenvalues of our tensor in Cartesian coordinates.
Doing this last step we obtain
\begin{align*}
